		...
		const result = streamText({
			...
			tools: {
				...
				addToCart: tool({
					...
					parameters: z.object({
						item: z.string().optional().describe("장바구니에 담을 메뉴 이름"),
						dummy: z.string().optional().describe("사용하지 않음")
					}),
					execute: async (args: any) => {
						// 🌟 AI가 item을 빼먹고 보내도 대화 내용이나 사용자 입력에서 유추하거나 기본값을 줍니다.
						const itemName = args.item || "애호박 전"; // 값이 없으면 기본적으로 애호박 전으로 처리!
						this.cart.push(itemName);
						console.log(`🛠️ [서버] addToCart 실행 (담은 메뉴: ${itemName})`);
						console.log(`🛒 [현재 장바구니]:`, this.cart);
						return { success: true, message: `${itemName}이(가) 장바구니에 담겼습니다.` };
					},
				} as any),
